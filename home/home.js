...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderContent() {
        ...
        switch (activeSection) {
            case 'RESERVAR':
                content = `
                    <h1 class="text-2xl text-white font-bold mb-6" tabindex="6">Salas e Laboratórios de Informática </h1>
                    <div class="mb-4">
                    ...
                    <div id="reservationForm" class="hidden mt-8">
                        <h2 class="text-xl text-white font-bold mb-4 sombra">Reservar <span id="labName"></span></h2>
            ...
            case 'SOLICITAÇÕES':
                content = `
                    <h1 class="text-2xl text-white font-bold mb-6" tabindex="6">Solicitações de Reserva</h1>
            ...
            case 'ADMINISTRAÇÃO':
                if (userType === 'Administrador') {
                    content = `
                            <h1 class="text-2xl text-white font-bold mb-6" tabindex="6">Área do Administrador</h1>
                            <div id="buttonsSection" class="bg-dynamicContent bordas-redondas shadow p-6 flex flex-col mt-16 mb-4">
                                <button id="PEDIDOS" class="sombra text-xl font-bold bg-blue-500 text-white text-base bordas-redondas w-full shadow-sm hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-300 mb-4 p-4 h-15" tabindex="7">Pedidos de Reserva</button>
                                <button id="SALAS" class="sombra text-xl font-bold bg-blue-500 text-white text-base bordas-redondas w-full shadow-sm hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-300 mb-4 p-4 h-15" tabindex="8">Gerenciar Salas</button>
                                <button id="USUARIOS" class="sombra text-xl font-bold bg-blue-500 text-white text-base bordas-redondas w-full shadow-sm hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-300 p-4 h-15" tabindex="9">Gerenciar Usuarios</button>
                            </div>
                            <div id="dynamicContent" class="bg-dynamicContent bordas-redondas shadow p-6 hidden" tabindex="6">
                            </div>
                                ...
                                <div class="relative top-80 mx-auto p-5 border w-96 shadow-lg bordas-redondas bg-white">
                                    <div class="mt-3 text-center">
                                        <div class="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-yellow-100">
                                            <img src="https://reserva-lab-nassau.s3.amazonaws.com/coroa.png" class="w-6 h-6">
                                        </div>
                    ...
                    <button id="backButton" class="bg-blue-500 text-white text-base font-medium bordas-redondas w-full shadow-sm hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-300 mb-4 p-4  h-15" tabindex="7">Voltar</button>
                    ${contentHtml}
                `;
                        ...
                        document.getElementById('PEDIDOS').addEventListener('click', () => {
                            resetFilters();
                            showContent(`
                                    <h1 class="text-2xl text-white font-bold mb-6" tabindex="8">Pedidos de Reserva</h1>
                                    <div class="flex flex-wrap justify-between items-center gap-4 mb-4">
                                        <button id="filterDropdownBtn" class="text-white bg-blue-500 p-2 bordas-redondas focus:outline-none w-full sm:w-auto">
                                            Filtros ☰
                                        </button>
                                        ...
                                        <div class="w-full sm:w-auto text-center">
                                            <button id="sortByIdButton" class="text-white bg-blue-500 p-2 bordas-redondas focus:outline-none w-full sm:w-auto" tabindex="9">
                                                Ordenar solicitação ▲
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    <div id="cancelConfirmationModal_Administrador" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden">
                                        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg bordas-redondas bg-white">
                                            <div class="mt-3 text-center">
                                                ...
                                                <h3 class="text-lg leading-6 font-medium text-gray-900 mt-5" tabindex="11">Confirmar Cancelamento</h3>
                                                <div class="mt-2 px-7 py-3">
                                                    <p class="text-sm text-gray-500">
                                                        Tem certeza que deseja cancelar esta reserva?
                                                    </p>
                        ...
                        document.getElementById('SALAS').addEventListener('click', () => {
                            resetFilters();
                            showContent(`
                                    <button id="CriarLab" class="SALAS bg-blue-500 text-white text-base font-medium bordas-redondas w-full shadow-sm hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-300 mb-4 p-4  h-15 tabindex="8">Criar Sala</button>
                                    <h1 class="text-2xl text-white font-bold mb-6" tabindex="9">Editar Salas</h1>
                                    <div class="mb-4">
                        ...
                        document.getElementById('USUARIOS').addEventListener('click', () => {
                            resetFilters();
                            showContent(`
                                    <h1 class="text-2xl text-white font-bold mb-6" tabindex="9">Gerenciar Usuarios</h1>
                                    <div class="mb-4">
    ...
    function setupReservationForm() {
        ...
        function validateTimeFim() {
            ...
            if (startTime && endTime) {
                ...
                // Nova validação para verificar se o horário de início é maior que 21:00 e o horário de término é maior que 22:00
        ...
        function validateStartTime() {
            ...
            if (startTime < minValidTime && selectedDate !== formattedYesterday) {
                ...
                timeInput.value = minValidTime; // Reseta para 7:00
            }
        ...
        await checkAuthToken(async () => {
            ...
            try {
                ...
                const filteredRequests = requests.filter(request =>
                    request.status === 'pendente' || request.status === 'aprovado' || request.status === 'concluído'
                );
    ...
    async function renderFilteredPedidos(requests) {
        ...
        if (filteredRequests.length === 0) {
            ...
            const sortRequests = () => {
                if (sortByStatus) {
                    filteredRequests.sort((a, b) => {
                        const statusOrder = { 'pendente': 1, 'aprovado': 2, 'concluído': 3 };
            ...
            const renderRequests = () => {
                const requestsHTML = filteredRequests.map(request => {
                    ...
                    return `
                    <div class="bg-white shadow-md bordas-redondas p-4 mb-4" tabindex="10">
                        <h3 class="font-bold text-lg mb-2">${request.lab_name}</h3>
                        <p class="text-sm text-gray-600">Solicitação: ${request.id}</p>
...
function showReservationForm(labName) {
    ...
    if (tableContainer) {
        tableContainer.innerHTML = '<p class="text-black font-bold">Selecione uma data para ver os horários disponíveis.</p>';
    }
...
function updateTable() {
    ...
    if (selectedDate) {
        tableContainer.innerHTML = '<p class="text-black font-bold">Carregando horários indisponíveis...</p>';
        ...
        currentFetchPromise.promise
            ...
            .catch(error => {
                ...
                tableContainer.innerHTML = '<p class="text-black font-bold">Erro ao carregar horários indisponíveis. Por favor, tente novamente.</p>';
            });
    } else {
        tableContainer.innerHTML = '<p class="text-black font-bold">Por favor, selecione uma data para ver os horários disponíveis.</p>';
    }
}
...
function renderUnavailableTimesTable(unavailableTimes, selectedDate) {
    ...
    if (filteredTimes.length === 0) {
        tableContainer.innerHTML = '<p class="text-green-500 font-bold">Todos os horários estão disponíveis para a data selecionada.</p>';
        return;
    }

    const tableHTML = `
        <h3 class="text-lg text-white font-bold mb-2 sombra">Horários já reservados para ${formatDate(selectedDate)}</h3>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ${filteredTimes.map((reservation, index) => `
                    <tr class="${index % 1 === 0 ? 'bg-white' : 'bg-white'}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm ${reservation.status === 'aprovado' ? 'text-green-600 bg-green-200' : reservation.status === 'utilizando' ? 'bg-utilizando' : 'text-yellow-600 bg-yellow-200'}">${reservation.status}</td>
                    </tr>
                    </tr>
                `).join('')}
            </tbody>
